Shader "KGame/Terrain Mesh Splatting Rain 1" {
...
void SplattingMeshTerrainUVs1(float2 uv, out float4 splattingPack0) {
    float2 maxUv = uv * _TerrainSize.zw;
    splattingPack0.xy = maxUv / _Size0[0];
    splattingPack0.zw = float2(0, 0);
}
...
v2f vert (appdata_full v) {
    ...
    SplattingMeshTerrainUVs1(uv, splattingPack0);
...
void SplattingMesh1(float4 splattingPack0, float2 splattingUV, out half3 albedo, out half3 localNormal, out half metallic, out half perceptualSmoothness, out half transparency) {
    half weight = 0.f;
...
void frag(v2f IN, out half4 color: SV_Target0) { 
    ...
    half transparency;
    SplattingMesh1(splattingPack0, splattingUV, albedo, localNormal, metallic, perceptualSmoothness, transparency);
